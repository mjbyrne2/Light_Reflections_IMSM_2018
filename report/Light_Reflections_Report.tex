...
In this work we have investigated several aspects of the light reflection. Our main goals are to 1) understand the diffusivity of the reflection associated with material of the surface, 2) construction and de-construction of complex geometric shapes using basic mathematical operation, 3) improve the current discrete model to continuous model favoring less data storage, and 4) extend the continuous model to incorporate the multi-path phenomenon. 

To achieve goal 2), we utilized the tool Rfunc, based on boolean operation, to help constructing complex shapes and obtaining functional representation of the surface that does not require storage of information such as vertices, unit normal vectors and triangle adjacency. To achieve 3), we learned to use the combination of Chebfun and indicator function, a computation tool that takes the advantage of Chebyshev polynomial interpolation, to perform numerical integration over the surface with less need of explicit expression. To achieve 4), we started from simple case of two-dimensional one reflection model and extended the result to build the thoery for three-dimensional n reflections model. These steps allow us to tap into 1) to gain a deeper understanding on the work of light reflection.

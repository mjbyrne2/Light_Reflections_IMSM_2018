...
\thanks{$^2$University of Alabama at Birmingham, sanogof1@uab.edu}
\author{Pai Song$^3$}
...
\begin{figure}[h]
\centering \includegraphics[scale=0.35]{./figs/OCS_ellipsoid}
...
\begin{figure}[h]
\centering \includegraphics[scale=0.35]{./figs/OCS_parallel_plane_ellipsoid}
...
\begin{figure}[h]
\centering \includegraphics[scale=0.22]{./figs/OCS_perpendicular_plane_ellipsoid}
...
\begin{figure}[h]
\centering \includegraphics[scale=0.2]{./figs/OCS_decay_by_alpha_ellipsoid}
...
To compute the contribution from lights that bounce multiple times before it is received, what we need is just to write the relations between the angles correctly. For the case for two bounces, we can deduce these relations with the help from the picture below. Observe that now
...
In this work we have investigated several aspects of the light reflection. Our main goals is to understand the diffusivity of the reflection associated with material of the surface. To do so first we construct and de-construct complex geometric shapes using Rvachev function which is based on boolean operations. Rvachev function help obtaining functional representation of the surface that does not require storage of information such as vertices, unit normal vectors and triangle adjacency.

...
is the so-called half-way vector. The parameter $\alpha \geq 0$ depends on the light reflectivity the material used to cover the surface (large reflectivity corresponds to large $\alpha$). Intuitively speaking, the more mirror-like an object is,  the shaper its specular peak will be (corresponding to an RDF that decays faster). \begin{figure}[H]
 \includegraphics[width=1.1\textwidth]{./figs/Surfaces.png}
\end{figure}
...
\begin{table}[H]
    ...
    \hline	
	1280 & $2.068455$ & $1.23853E-02$ & 0.98381 \\
    \hline	
...
To capture the target area for numerical integration using Chebfun, instead of defining the explicit boundary angles, the multiplication to the integrand by $max\{0, \langle L,N\rangle\}$ and $max\{0,\langle V,N\rangle\}$ is used. This excludes the area outside of the intersection of the lighted area and viewer's range. Notation $\langle\cdot, \cdot\rangle$ is the inner product, $L$ is the light source direction, $V$ is the viewer direction, and $N$ is the outward unit normal of the surface.

\subsubsection{Verification of the method}

Verification of the accuracy of Chebfun is performed by simulating over an unit sphere. The light source and the viewer are placed at the identical direction. The reflexivity and the light diffusivity of the surface are fixed as $\rho = 1$ and $\alpha =0$ respectively. The optical cross section (OCS) approximated using Chebfun, with interpolating error tolerance of $10^{-8}$, yields $\sigma_{cheb} = 2.094395102389723$. The known analytic solution is $\sigma_{analytic} = \frac{2*\pi}{3}\approx 2.094395102393195$. The absolute error of the Chebfun approach is in the range of $10^{-12}$. This is superior to the triangular discretization approach which performs at $\approx 10^{-4}$ accuracy. To ensure that the Chebfun approach produces physical results, additional tests were run for different $\alpha$ value and different viewer's position.

The $\alpha$ value is chosen from $[0,18]$ with fixed increment. The results shows that the OCS value decreases as $\alpha$ increases (Fig \ref{OCSParallelPlane}). Because high $\alpha$ value indicates small specular highlight, the viewer would receive less total reflected light across the whole surface. This is in agreement with the physical observation.

The influence of the viewer position is tested by simulating parallel orbiting viewer and perpendicular orbiting viewer with respect to the light source. For the parallel orbiting scenario (Fig. \ref{OCSParallelPlane}), the OCS value decreases as the distance between the light source and the viewer increases, followed by restoration to the initial value. The rate of decay is influenced by the magnitude of the $\alpha$ value. For the perpendicular orbiting scenario, the viewer moves on the xy-plane while the light source is fixed at $(\theta, \phi) = (0,0)$ position. The simulation results shows that the OCS value remains constant (Fig. \ref{OCSPerpendicularPlane}).

Both orbiting simulations resulted in agreement with the physical phenomenon. This indicates that Chebfun approach can produce accurate and physical results given a spherical surface.
...
\subsubsection{Case 2: Ellipsoid I} In this case we applied the Chebfun approach to a triaxial ellipsoid due to its simple geometry and non-uniform symmetry. We started with the ellipsoid $\frac{x^2}{4}+\frac{y^2}{9}+\frac{z^2}{1} = 1$. $\rho$ and $\alpha$ are as in the previous section. Due to the artificial singularities generated from the surface parametrization, small neighborhoods near the "north pole" and the "south pole" are intentially excluded from calculation. Light source is fixed at $(\theta,\phi) = (\pi/2, \pi/2)$ (parallel to the long axis) and the viewer orbits on the xy-plane. The Chebfun approach yields OCS value $\approx 0.9821$ when the viewer and the light source are at the identical direction. This lowered OCS value is likely due to the difference in curavtures of the surface facing $(\theta, \phi)=(\pi/2, pi/2)$ comparing to a hemispherical surface. Similar behavior of the decay of OCS value is observed and the ellipsoid case displays faster decay (Figure \ref{OCSParallelPlane} and Figure \ref{OCSParallelPlaneEllipsoid}). We also performed the same test simulating viewer orbiting on a plane perpendicular to the light source, the resulting periodic oscillation obtained is determined to be physical.
...
\caption{Simulation of orbiting viewer on the plane perpendicular to the light source. $\alpha = 0$. The shape of interest here is an ellipsoid. The color code is identical to Figure  \ref{OCSParallelPlaneEllipsoid}.} 
...
\subsubsection{Case 3: Ellipsoid II} We next chose the ellipsoid described by $\frac{x^2}{9}+\frac{y^2}{4}+\frac{z^2}{1} = 1$ to compare the result with ellipsoid I case. The remaining setup is identical to the previous case (light source parallel to the short axis). Using the Chebfun approach we found that the OCS value when the light source and the viewer are at the identical position is $\approx 1.6786$. Since the viewer would be facing a larger area, more reflected light are accumulated comparing to the ellipsoid I case. Finally we looked at the decay rate of the OCS value based on the increase of specular highlight. The result indicates that even though the geometry has definite influence on how fast the OCS decreases, but the rate of change of the values would eventually converge. The effect of difference in curvature and light source position can be seemed in Figure \ref{OCSPerpendicularPlaneEllipsoid}.
...
Let's now consider the case where light projected on a concave surface. Unlike the previous cases, the light detected by the sensor can be reflected multiple times from points on the surface other points on the surface due to concavity. Theoretically, the bouncing of light can go on infinitely if there is no energy dissipation. In reality however, light will only be reflected finitely many times before it completely dissipates. The surface we consider here is the half cylindrical shell facing downwards and the radius of the cylinder is 1. The configuration is shown here. 

\begin{figure}[H]
  \includegraphics[width=4.5in]{./figs/multipath_edit.png}
  \label{fig:reflection}
\end{figure}

Given $\psi_I$, we suppose incoming light beam is denoted by $\mathbf{I}=(\cos\psi_{I},\sin\psi_{I})$ and is reflected by the surface at $D_1,D_2,...,D_n$ before it is collected by the light detector whose viewing angle is $\mathbf{V}$. Denote the (inward) normal vector at $D_n$ by $\mathbf{N}_n=(-\cos\vartheta_n,-\sin\vartheta_n)$ and unit vector for $\overrightarrow{D_{n}D_{n-1}}$ by $\mathbf{I}_n=(-\cos\psi_n,-\sin\psi_n)$ with $\psi_n$ defined inductively as  
...
\begin{proposition}
The OCS with reflections on the cylindrical surface of radius 1 is computed by
...
under the assumption that the infinite sum converges.
\end{proposition}

\begin{proof}
By Blinn-Phong model, the contribution from a single bounce to the OCS is computed by
...
Let's now consider the contribution due to two bounces. From the picture above, it is clear to see that
$\angle{ID_1O}=\angle{OD_1D_2}=\angle{D_1D_2O}:=\delta_1$. Thus $\vartheta_2-\vartheta_1=\angle{D_2OD_1}=\pi-2\angle{OD_1D_2}=\pi-2\delta_1$. On the other hand we have $\psi_1=\psi_2+2\delta_1$. Thus
...
The extra factor  $1/\|\mathbf{N}_1-{\mathbf{N}}_2\|^2$ in the integrand is to account for energy lost when light travels long distances. To get contribution for three bounces, simply view $I_2$ as the incident light compute similarly. 
\end{proof}

\section{Conclusion}
\section{acknowledgement}
The authors thank North Carolina State University for hosting the workshop and are grateful to Prof. Mansoor Haider and Thomas Gehrmann for organizing. The authors also owe greatly to John Peach for introduction into the project and guidance  throughout and to Prof. Alen Alexanderian for his patience and kind instructions. 

...
\begin{figure}[h]    	\centerline{\includegraphics[height=2in]{./figs/sdh_openscad}
     	\hspace{ 6pt}
		\includegraphics[height=2in]{./figs/sdh_mesh}}
     	\hspace{-6pt}
		\caption{Constructive Solid Geometry \& Generation of finite element mesh  \cite{trimesh}}
        \label{fig:sdh_mesh}
\end{figure}
...
\subsection{Numerical calculation of Scattering function - finite element approach}~\\
...
\begin{figure}     	\centerline{\includegraphics[height=1.6in]{./figs/facet.png}}
...
\end{itemize} 
The following table \ref{table: sphere OCS} shows the numerical results for different mesh size of a unit sphere with constant $\rho=1$ and $\alpha=0$. The analytic OCS for a sphere is $2\pi/3\approx 2.094395102393195$.
\begin{table}[htp]
	\small
	...
	\hline
	320 & $1.992939$ & $4.84419E-02$ &  \\
    \hline	
	1280 & $2.068455$ & $1.23853E-02$ & 0.98381 \\
    \hline	
	5120 & $2.087873$ & $3.11387E-03$ & 0.99593 \\
	\hline
	20480 & $2.092762$ & $7.79570E-04$ & 0.99898 \\
	\hline
	\end{tabular}
...
%{\bf To capture the target area for numerical integration using Chebfun, the boolean relationship is chosen to avoid explicit boundary angles. This is achieved by multiplying the integrand derived from the Blinn-Phong model by $max\{0, \langle L,N\rangle\}$ and $max\{0,\langle V,N\rangle\}$ which rules out area not in the intersection of the lighted area and viewer's range. Here $\langle\cdot, \cdot\rangle$ is the inner product, $L$ is the light source direction, $V$ is the viewer direction, and $N$ is the outward unit normal of the surface.}
...
{\bf CASE 2: Ellipsoid I} Following the verification we applied the Chebfun approach to a non-uniform shape. The shape chosen is an triaxial ellipsoid due to its simple geometry and non-uniform symmetry. We started with the ellipsoid $\frac{x^2}{4}+\frac{y^2}{9}+\frac{z^2}{1} = 1$. $\rho$ and $\alpha$ are as in the verification section. However, due to the artificial singularities generated from the parametrization of the surface, we intentially excluded a small neighborhood near the "north pole" and the "south pole" to avoid instability. Light source is fixed at $(\theta,\phi) = (\pi/2, \pi/2)$ and the viewer orbits on the xy-plane. The Chebfun approach yields OCS value of $\approx 2.1104$ when the viewer is positioned in the same direction as the light source. The similarity of this OCS value may be due to the comparable geometry of the surface facing $(\theta, \phi)=(\pi/2, pi/2)$ to a hemisphere.Similar to the sphere case, the OCS value drops down as the viewer moves further away from the light source. The decay rates of OCS value between the sphere and ellipsoid case are expected to be different, where the ellipsoid case is with the higher rate, and can be observed using Figure \ref{OCSParallelPlaneEllipsoid} and Figure \ref{OCSPerpendicularPlaneEllipsoid}. We also performed the same test simulating viewer orbiting on a plane perpendicular to the light source, the resulting periodic oscillation obtained is determined to be physical.

{\bf CASE 3: Ellipsoid II} We next chose the ellipsoid described by $\frac{x^2}{9}+\frac{y^2}{4}+\frac{z^2}{1} = 1$ to compare the result with ellipsoid I case. The remaining setup is identical to the previous case. Using the Chebfun approach we found that the OCS value when the light source and the viewer are at the identical position is $\approx 1.1927$. Since the viewer would be facing a larger area, the OCS value is expected to be higher. The result turns out to be the exact opposite such that the OCS value produced is lower than the previous case. However in terms of the decay rate of the OCS value, when the viewer moves away from the light source, the curvature change is less drastic compared to the ellipsoid I case.

\begin{figure}
\includegraphics[scale=0.25]{./figs/OCS_parallel_plane}    
\caption{Simulation of orbiting viewer on the plane as the light source with different $\alpha$ values.}    
\label{OCSParallelPlane}
\end{figure}

\begin{figure}
\includegraphics[scale=0.25]{./figs/OCS_perpendicular_plane}
...
\begin{figure}
\includegraphics[scale=0.25]{./figs/OCS_parallel_plane_ellipsoid}
...
\begin{figure}
\includegraphics[scale=0.25]{./figs/OCS_perpendicular_plane_ellipsoid}
...
\begin{figure}
\includegraphics[scale=0.25]{./figs/OCS_decay_by_alpha_ellipsoid}
...
$$\sigma_2 =\int_{0}^{\pi}\int_{0}^{\pi}\Bigg(\frac{\ip{\mathbf{I}_2+\mathbf{N}_2}{\mathbf{V}}}{\|\mathbf{I}_2+\mathbf{N}_2\|}\Bigg)^\alpha\frac{\ip{\mathbf{N}_2}{\mathbf{V}}}{\|\mathbf{N}_2\| \|\mathbf{V}\|}\frac{\ip{\mathbf{N}_2}{\mathbf{I}_2}}{\|\mathbf{N}_2\| \|\mathbf{I}_2\|} \frac{1}{\|N_1-N_2\|^2}\mathrm{d}\vartheta_1\mathrm{d}\vartheta_2$$
where the extra factor  $1/\|N_1-N_2\|^2$ is to due to the fact that the longer distance the light travels from $D$ to $E$, the weaker the light can be gathered at $E$. Summing the two gives an analytical formula for the whole OCS

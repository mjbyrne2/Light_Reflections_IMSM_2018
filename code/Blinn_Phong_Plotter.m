...
blinn_phong_MRDF = @(V,alpha) ((dot(V+V,N)./norm(V+V)).^alpha)...
    .*(dot(N,V)^2);   % Simplified since I = V
...
for j = 1:length(A)
    ...
    for k = 1:length(theta)
        B(j,k) = blinn_phong_MRDF(V(:,k),A(j));
    end
...
legend([Q(5) Q(6) Q(1) Q(2) Q(3) Q(4)],...
    ...
    'Location','Northwest')
set(gca,'Fontsize',20)
print('-depsc', 'BRDF_Vectors') % Save figure

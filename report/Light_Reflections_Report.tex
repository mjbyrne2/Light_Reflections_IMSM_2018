...
We can compute the total contribution for more bounces in similar fashion. Assume the incoming light $\mathbf{I}$ is reflected by the surface at $D_1,D_2,...,D_n$ before it is collected by the light detector whose viewing angle is $\mathbf{V}$. Denote the normal vector at $D_n$ by $N_n=(\cos\vartheta_n,\sin\vartheta_n)$ and vector for $\overrightarrow{D_{n}D_{n-1}}$ by $\mathbf{I}_n$. Define inductively 
